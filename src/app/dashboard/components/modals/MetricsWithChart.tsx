...
const MetricsWithChart = ({
  ...
  const readRateDiff = Number(metrics.labels.ideal_read) - Number(metrics.labels.read)
  const hashRateDiff = Number(metrics.labels.ideal_hash) - Number(metrics.labels.hash)
  ...
  return (
    <div className="fixed inset-0 bg-gray-600/50 overflow-y-auto w-full">
      ...
      <h2 className="mb-10 text-center text-2xl font-bold">Read and Hash Rate Data</h2>
        <div className="flex justify-between items-start mt-6">
          <div>
            <h4 className="text-lg mb-1">Read Information</h4>
          ...
          <div>
            <h4 className="text-lg mb-1">Hash Information</h4>
            <div className="text-sm">
              <div>
                Current:{" "}
                <span className="font-semibold">
        ...
        <div className="h-60 md:h-80 py-10 my-10 flex justify-between">
          <div className="">
            <h4>Read Rate</h4>
            {readRateDiff > 0 ? <Doughnut data={readData}/> :<p className="pr-10 text-gray-400 mt-20"><i>Current read rate is greater than ideal read rate</i></p>}
          </div>

          <div>
            <h4>Hash Rate</h4>
            {hashRateDiff > 0 ? <Doughnut data={hashData}/> :<p className="pr-10 text-gray-400 mt-20"><i>Current hash rate is greater than ideal hash rate</i></p>}
          </div>
        </div>
